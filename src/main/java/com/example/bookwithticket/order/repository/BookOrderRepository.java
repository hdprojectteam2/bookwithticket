...
public interface BookOrderRepository
    ...
    /* 주문번호와 회원 번호 검증 */
    Optional<BookOrderEntity> 
    findByOrderNumberAndMemberId(String orderNumber, Long memberId);
    ...
    List<BookOrderEntity> 
    findByMemberIdAndOrderStatusOrderByCreatedAtDesc(Long memberId, OrderStatus orderStatus);

    @EntityGraph(attributePaths = {"orderItems", "orderItems.book"})
    List<BookOrderEntity> 
    findByMemberIdAndOrderStatusAndDeletedFalseOrderByCreatedAtDesc(Long memberId, OrderStatus orderStatus);

    List<BookOrderEntity> 
    findByMemberIdAndOrderStatusInAndDeletedFalseOrderByCreatedAtDesc(Long memberId, List<OrderStatus> orderStatuses);
}